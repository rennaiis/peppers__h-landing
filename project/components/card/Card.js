...
export default function Card({isOpened, bgItems, handleClick, mod, icon, cover, title, ol, text, boldText, fact, ul, table}) {
  return (
    <div className={classNames(styles[`card_${mod}`], styles.card, {[styles.card_opened]: isOpened})} onClick={()=>handleClick(mod)}>
      <div className={styles.card__bg}>
        {bgItems?.map((item, index)=>{
          return(
            <div key={index} className={classNames(styles[`card__bgItem_${item.mod}`], styles.card__bgItem)}>
              <Picture {...item}/> 
            </div>
          )
        })}
